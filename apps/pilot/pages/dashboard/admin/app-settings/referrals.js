...
import { approveAffiliate } from "../../../../config/utilityFunctions";
  ...
  return (
    <AdminAppSettings>
      <div className="max-w-screen-md">
        ...
        {/* Content */}
        <div className="w-full p-3 pb-8 mt-3 custom-shadow-xs rounded-md bg-white">
          <div className="flex items-center gap-x-2">
            {loading && (
              <LoadingSpinner width={5} height={5} color={"navyBlue"} />
            )}
          ...
          <div className="mt-5 mx-2 flex flex-col gap-y-4">
            {requests.length > 0 &&
              requests.map((request) => (
                <div
                  key={request.id}
                  className="w-full bg-white rounded-md custom-shadow-xs p-4"
                >
