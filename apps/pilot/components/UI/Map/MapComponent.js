...
import { getPolygonCenter } from "../../../config/utilityFunctions";

const MapComponent = ({ center, zoom, polygon, area, price, onlyAmount }) => {
  ...
  useEffect(() => {
    ...
    const getMapCenter = async (coordinates) => {
      try {
    ...
    getMapCenter(polygon.features[0].geometry.coordinates);
