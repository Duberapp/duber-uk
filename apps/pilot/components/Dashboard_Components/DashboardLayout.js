...
import { FullScreenLoading, JobDetails_Sidebar } from "../";
...
const DashboardLayout = ({
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      const { data: billingData, error: billingDataErr } =
        await getBankInformation(data[0].id);
      if (billingDataErr) return;

      dispatch(setCurrentUser(data[0]));
      dispatch(setUserBilling(billingData));
      ...
      >
        ...
        <div className="fixed lg:flex hidden min-h-screen  overflow-y-hidden bg-white w-[250px] px-7 py-5 flex-col justify-between">
          <div className="">
            ...
            <div className="mt-8 flex flex-col gap-3">
              ...
              {isAdmin && (
                <>
                  ...
                  {/* Admin* - App Settings */}
                  <Link href={"/dashboard/admin/app-settings"} legacyBehavior>
                    <a
                      onClick={() => {
                        toast.remove();
                        dispatch(setActiveJob(null));
                      ...
                      className={`sidenav-item ${
                        activeNav == "dashboard/admin/app-settings" && "active"
                      }`}
                    >
                      <ShieldCheckIcon className="w-6 h-6" strokeWidth={2} />
                      <p className="text-base font-semibold">App Settings</p>
                    </a>
                  </Link>
                </>
              )}
            ...
            <Link href="https://support.duber.uk/" className="mt-4">
              <a
                className={`sidenav-item ${
