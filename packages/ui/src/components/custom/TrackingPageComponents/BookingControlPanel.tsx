...
export default function BookingControlPanel({
  ...
  return (
    <div
      className={`relative w-full h-full  overflow-hidden flex flex-col rounded-lg ${className}`}
    >
      <div
        className={`w-full flex-1 z-20 flex flex-col items-center sm:justify-end justify-between ${
          deliverablesView ? "p-0" : "p-2 h-full"
        }`}
      >
        {!showCancelBookingPanel &&
        ...
        {showCancelBookingPanel && (
          <CancelBookingPanel
            handleSetClose={() =>
              setShowCancelBookingPanel && setShowCancelBookingPanel(false)
            }
            handleCancelBooking={handleCancelBooking!}
          />
        )}
    ...
    <div className="w-full h-full bg-white rounded-lg flex flex-col p-3">
      <div className="flex-1">
        ...
        <div className="w-full p-3 rounded-lg bg-red-200 text-red-600 text-[14px]">
          ...
          <br />
          <br />
          Canceling this booking means you understand that the booking will be
          terminated and the <span className="font-semibold">deposit</span>{" "}
          already paid is <span className="font-semibold">non-refundable</span>
        </div>
      </div>
  ...
  return (
    <div className="w-full h-full bg-white rounded-lg flex flex-col p-3">
      <div className="flex-1 w-full p-3 rounded-lg bg-red-200 text-red-600 text-[12px]">
        ...
        <br />
        <br />
        The pilot assigned to this booking has cancelled this booking due to “
        <span className="font-semibold">{reason}</span>”
        <br />
        <br />
        Cancellations via pilots are rare, however they do happen and in most
        cases its due to them determining its not safe and will not be able to
        provide you with the best possible service.
        <br />
        <br />
        The deposit paid, will be fully refunded and the scheduled payment will
        be revoked.
        <br />
        <br />
        ...
        <br />
        <br />
        If you feel like this cancellation was unjust, please contact support on
        02392 1797 93
      </div>
    </div>
  );
...
function PilotDetaisPanel({
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-lg flex flex-col p-3">
      ...
      <div className={`w-full flex justify-between items-center`}>
        <div
          className={`flex flex-1 gap-x-3 ${
            showMore
              ? "flex-row-reverse items-start justify-between"
              : "items-center"
          }`}
        >
          <img
            src={
              pilotData?.profilePic
                ? getPilotProfilePictureLink(pilotData.profilePic)
                : "/assets/avatar.jpg"
            }
            alt=""
          ...
          <div className="">
            <h2 className="font-semibold text-base text-duber-navyBlue">
              Pilot {showMore ? "Name" : "Assigned"}
            </h2>
            <h2 className="text-sm text-duber-navyBlue">
              {isPilotAssigned
        ...
        {pilotData?.telNumber && !showMore && (
          <a
            href={`tel:${pilotData.telNumber}`}
            className="p-3 rounded-lg bg-duber-skyBlue"
          >
      ...
      {showMore && (
        <div className="">
          <h2 className="text-sm font-semibold text-duber-navyBlue">
            Expertise
          </h2>
          <p className="mt-1 text-xs text-duber-navyBlue">
            {pilotData.pilot_skill?.map((skill, index) => (
              <span key={index}>
                {
                  pilot_skills.filter(
                    (skillData) => skillData.slug === skill
                  )[0].title
                }
                ,{" "}
              </span>
          ...
          <h2 className="mt-3 text-sm font-semibold text-duber-navyBlue">
            Drone Equipment
          </h2>
          <p className="mt-1 text-xs text-duber-navyBlue">
            {pilotData.userDrones &&
              pilotData.userDrones?.length > 0 &&
              pilotData.userDrones?.map((drone: any) =>
                !drone ? (
                  <></>
                ) : (
                  <span key={drone}>
          ...
          <p className="mt-1 text-xs text-duber-navyBlue">
            Operator ID : {pilotData.operatorID}
          </p>
          <p className="mt-1 text-xs text-duber-navyBlue">
            Flyer ID : {pilotData.flyerID}
          </p>

          <div className="flex items-center gap-x-2 mt-4">
            <Button
              onClick={() => handleDownloadProof({ type: "training" })}
              variant={"pink-outline"}
              className="flex-1 px-2 h-12 rounded-lg"
            >
              <div className="flex w-full items-center">
                <FileDown className="w-6 h-6 text-duber-pink" strokeWidth={2} />
                <p className="whitespace-nowrap flex-1 text-center text-xs font-semibold text-duber-pink">
                  Training Proof
                </p>
              </div>
            </Button>
            ...
            <Button
              onClick={() => handleDownloadProof({ type: "insurance" })}
              variant={"pink-outline"}
              className="flex-1 px-2 h-12 rounded-lg"
            >
              <div className="flex w-full items-center">
...
function DeliverableItem({
  ...
  return (
    <div key={id} className="w-full flex flex-col items-center">
      ...
      <a href={link} className="mt-1 text-[10px] text-duber-navyBlue">
        {name}
      </a>
    </div>
  );
    ...
    <div className="w-full h-full flex flex-col gap-y-3 bg-gray-200 p-2.5">
      <div className="relative w-full h-full flex flex-col gap-y-3">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-duber-navyBlue font-semibold text-lg">
            Deliverables
          </h2>
          ...
          <Button
            ...
            disabled={isDeliverablesExpired}
          >
        ...
        {/* Deliverable List */}
        <div
          className={`flex-1 w-full grid grid-cols-3 gap-2 ${
            isIssuesShowing ? "max-h-[10rem]" : "max-h-[16rem]"
          } overflow-y-scroll`}
        >
      ...
      <PilotRatingCard
        pilotData={pilotData}
        orderData={orderData}
        setIsIssuesShowing={setIsIssuesShowing}
      />
    </div>
  );
}

function PilotRatingCard({
  pilotData,
  setIsIssuesShowing,
  orderData,
}: {
  ...
  const [ratingStatus, setRatingStatus] = useState<RatingStatusType | null>(
    null
  );
  const [submittingRate, setSubmittingRate] = useState<boolean>(false);
  const [initializing, setInitializing] = useState<boolean | "initialized">(
    false
  );
  ...
  useEffect(() => {
    if (pilotRate === 1) {
      ...
      pilotRate === 2
        ? setRatingStatus("Poor")
        : pilotRate === 3
        ? setRatingStatus("Okay")
        : pilotRate === 4
  ...
  async function triggerFeedbackRequest() {
    try {
      ...
      const { data: exsistingData, error: exsistingError } =
  ...
  return (
    <div className="w-full p-2.5 bg-white rounded-lg">
      <h2 className="text-duber-navyBlue font-semibold text-lg mb-3">
        Rate Pilot
      </h2>

      <div
        className={`relative flex gap-x-3 ${
          showIssuePanel ? "items-start" : "items-center"
        }`}
      >
        ...
        <div className="">
          <div className="flex items-center justify-between">
            <p className="text-duber-navyBlue text-[13px]">
              {pilotData.firstName} {pilotData.lastName}
            </p>
            {ratingStatus && (
              <p className="text-duber-navyBlue text-base font-semibold flex items-center gap-x-1">
                {submittingRate && (
                  <Loading className={"w-4 h-4 text-duber-navyBlue"} />
                )}{" "}
                {ratingStatus}
            ...
            <div className="mt-3">
              ...
              <div className="mt-1 flex items-center flex-wrap gap-1">
                {pilotRateIssuesList.map((issue) => (
                  <div
                    key={issue.id}
                    className={`cursor-pointer px-2.5 py-1.5 border-2 border-gray-400 rounded-lg text-[10px] ${
                      pilotIssue === issue.issue && "bg-gray-400 text-white"
                    }`}
