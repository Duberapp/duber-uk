...
export default function CustomerCard({ name, company, totalBookings, customerID, subscriptions, userType, onView }: Props) {
  return (
    <div className='w-full p-2 rounded-lg bg-white shadow-md flex items-center justify-between border border-slate-200'>
      ...
      <div className="flex-1 items-center justify-between px-6 flex">
        ...
        <div className='flex-1'>
          <p className="text-xs text-duber-navyBlue font-light">Total Bookings</p>
          <p className="text-base text-duber-navyBlue">{totalBookings ? totalBookings : "None"}</p>
        </div>
        <div className='flex-1'>
          <p className="text-xs text-duber-navyBlue font-light">User Type</p>
          <p className="text-base text-duber-navyBlue normal-case">{!userType ? "None" : userType === 'account' ? "Account" : userType === 'guest' && "Guest"}</p>

        </div>
        <div className='flex-1'>
          <p className="text-xs text-duber-navyBlue font-light">Subscriptions</p>
          <p className="text-base text-duber-navyBlue">{subscriptions ? subscriptions : "None"}</p>
        </div>
        <div className='flex-1'>
          <p className="text-xs text-duber-navyBlue font-light">Customer ID</p>
          <p className="text-base text-duber-navyBlue">{customerID ? customerID : "None"}</p>
        </div>
      </div>

      <Button size={'xxl'} variant={'teal'} onClick={() => onView(customerID!)}>View</Button>
    </div>
  )
}