...
export default function FilterDropdown({ dropdownLabel, items, placeholder, setValue, value }: FilterDropdownProps) {
  ...
  useEffect(() => {
    ...
    const filteredItem = items ? items.filter(item => item.slug === value) : null;
    ...
    <div className='flex items-center gap-x-2'>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant={'outline'} className='rounded-full flex items-center gap-x-3'>
            <span>{label}</span>
        ...
        <DropdownMenuContent className='w-56'>
          ...
          <DropdownMenuRadioGroup value={value} onValueChange={setValue}>
            ...
            {value && (
              <>
                <DropdownMenuSeparator />
                <DropdownMenuRadioItem value={''} className='flex items-center gap-x-1'>
